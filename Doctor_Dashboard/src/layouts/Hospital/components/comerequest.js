...
const Comerequest = () => {
  ...
  const fetchRequestedhospitals = async () => {
    try {
      const { data } = await axios.get('http://localhost:3005/doctor/requested-hospitals');
  ...
  const approvehospital = async (hospital) => {
    try {
      await axios.post('http://localhost:3005/api/approve-hospital', { hospitalID: hospital.hospitalID });

      // Fetch the updated list of approved hospitals
      await updatehospitalsList();
      
      setRequestedhospitals(requestedhospitals.filter(d => d.hospitalID !== hospital.hospitalID));
    } catch (error) {
  ...
  const rejecthospital = async (hospital) => {
    try {
      await axios.post('http://localhost:3005/api/reject-hospital', { hospitalID: hospital.hospitalID });

      // Remove the hospital from requested hospitals list
      setRequestedhospitals(requestedhospitals.filter(d => d.hospitalID !== hospital.hospitalID));
    } catch (error) {
      ...
      <table className="table table-striped ">
        ...
        <thead className="sticky-top" style={{ zIndex: 1, backgroundColor: '#cce0ff' }}>
                  <tr style={{ cursor: 'pointer' }}>
                    ...
                    <th onClick={() => handleSort('Role')} style={{ backgroundColor: '#cce0ff', fontSize: '14px', fontWeight: '200px', color: 'black' }}>
                    ...
                    <th onClick={() => handleSort('ContactNo')} style={{ backgroundColor: '#cce0ff', fontSize: '14px', fontWeight: '200px', color: 'black' }}>
                    ...
                    <th onClick={() => handleSort('WTime')} style={{ backgroundColor: '#cce0ff', fontSize: '14px', fontWeight: '200px', color: 'black' }}>
                    ...
                    <th onClick={() => handleSort('Message')} style={{ backgroundColor: '#cce0ff', fontSize: '14px', fontWeight: '200px', color: 'black' }}>
        ...
        <tbody>
          {currentrequestedhospitals.map((hospital) => (
            <tr key={hospital.hospitalID}>
              ...
              <td>
                <button className="btn btn-success mr-2" onClick={() => approvehospital(hospital)}>
                  Approve
                </button>
                <button className="btn btn-danger" onClick={() => rejecthospital(hospital)}>
                  Reject
                </button>
              </td>
            </tr>
