...
const SignUp1 = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:3005/doctor/doctorlogin", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        if (responseData.success) {
          const { Doctor_id, userEmail } = responseData;
  
          // Save Doctor_id in sessionStorage
          ...
          navigate("/app/dashboard/default", { state: { userEmail, Doctor_id } });
        } else {
