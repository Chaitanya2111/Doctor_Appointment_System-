...
function Departments() {
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:3005/hospital/hdepartments/${hospitalId}`);
  ...
  const handleEdit = (dep_id) => {
    navigate(`/adddepartments/${dep_id}`);
  };
      ...
      <div className="content">
        <div className="row">
          ...
          <div className="col-lg-10 col-6 text-right m-b-20 d-flex justify-content-end">
            <Link to={'/adddepartments'} className="btn btn btn-success btn-rounded float-right">
              ...
              <table className="table table-border datatable mb-0" style={{ width: '100%' }}>
                ...
                <thead className="sticky-top" style={{ zIndex: 1, backgroundColor: '#ccfff5' }}>
                  <tr style={{ cursor: 'pointer' }}>
                    <th onClick={() => handleSort('department_name')} style={{ backgroundColor: '#ccfff5', fontSize: '14px', fontWeight: '200px', color: 'black' }}>
