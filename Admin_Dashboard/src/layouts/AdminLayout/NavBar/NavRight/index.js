...
const NavRight = () => {
  ...
  useEffect(() => {
    // Get Hospital_id from session storage
    const hospitalId = sessionStorage.getItem('Hospital_id');
    if (hospitalId) {
      // Fetch contact details based on Hospital_id
      fetchContactDetails(hospitalId);
    }
  }, []);

  // Function to fetch contact details from API
  const fetchContactDetails = async (hospitalId) => {
    try {
      const response = await fetch(`http://localhost:3005/hospital/hospitalinfo/${hospitalId}`);
      if (response.ok) {
        ...
        <ListGroup.Item as="li" bsPrefix=" ">
          ...
          <Dropdown.Toggle as={Link} variant="link" to="#" id="dropdown-basic" style={{color:'#ffffff'}}>
            ...
            <Dropdown.Menu align="end" className="profile-notification">
              {/* User Profile Header */}
              <div className="pro-head" style={{backgroundColor:'#00b38f'}}>
                {contactInfo && (
                  <>
                    <img src={avatar1} className="img-radius" alt="User Profile" />
                    <span>{contactInfo.NameOfHospital}</span>
                  </>
                )}
                {/* Logout Button */}
              ...
              <ListGroup as="ul" bsPrefix=" " variant="flush" className="pro-body">
                ...
                <ListGroup.Item as="li" bsPrefix=" ">
                  {/* Profile Link */}
                  <Link to="/hospitalprofile" className="dropdown-item" onClick={goToProfile}>
